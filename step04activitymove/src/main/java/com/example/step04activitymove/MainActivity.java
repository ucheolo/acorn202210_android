...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View view) { // View view 에는 눌러진 Button 객체의 참조값이 들어 있다.
        /*
        // 만일 해당 참조값이 toCanada 라는 필드에 있는 값과 같다면 Canada 로 가기 버튼을 누른것이다.
        if(view == toCanada){
            // CanadaActivity 를 활성화 시킬수 있는 의도를 가진 객체 생성
            Intent intent = new Intent(this, CanadaActivity.class);
            startActivity(intent);
        }else if(view == toGerman){ // 만일 해당 참조값이 toCanada 라는 필드에 있는 값과 같다면 German 로 가기 버튼을 누른것이다.
        ...
        */
        // 눌러진 버튼의 아이디값(자동 부여된 정수값)을 읽어와서 R 클래스의 등록된 아이디값과 비교해서 분기하기
        switch (view.getId()){
            case R.id.toCanada:
                // CanadaActivity 를 활성화 시킬수 있는 의도를 가진 객체 생성
                Intent intent = new Intent(this, CanadaActivity.class);
